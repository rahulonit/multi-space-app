...
final class AIService: ObservableObject {
    ...
    func streamCoPilotResponse(
        ...
        if let ct = channelType, !ct.isEmpty {
            groupContextSection += "• Channel / Chat Type: \(ct)\n"
        }
        if let mi = membersInfo, !mi.isEmpty {
            groupContextSection += "• Group Members & Roster Info:\n\(mi)\n"
        }
        ...
        var userPrompt = ""
        if !groupContextSection.isEmpty {
            userPrompt += "=== VERIFIED CONVERSATION ROSTER & METADATA ===\n\(groupContextSection)\n\n"
        }
        if !cleanContext.isEmpty {
            userPrompt += "=== CONVERSATION THREAD TRANSCRIPT ===\n\(cleanContext)\n\n"
        }
        userPrompt += "=== USER REQUEST / QUESTION ===\n\(cleanPrompt)"
        ...
        if lowPrompt.contains("admin") || lowPrompt.contains("member") || lowPrompt.contains("group") || lowPrompt.contains("who is in") || lowPrompt.contains("participants") {
            ...
            answer += "*Analysis generated by PINGGO Smart Engine.*"
            onChunk(answer)
            return answer
        }
